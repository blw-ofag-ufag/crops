document.addEventListener('DOMContentLoaded', () => {
    ...
    // Refactored Query to infer dataset membership dynamically
    const CONSTRUCT_QUERY = `
        ...
        WHERE {
            ...
            OPTIONAL {
                ...
                FILTER(?membershipName != "Unknown")

                OPTIONAL { ?membership schema:validFrom ?validFrom . }
                OPTIONAL { 
                    ?membership schema:validTo ?validTo .
                    # Exclude undefined boundaries to maintain data integrity
                    FILTER(?validTo != cube:Undefined)
                }
            }
            OPTIONAL {
    ...
    // Removed strict 'xsd:date' typing for validity ranges to support pure gYear literals
    const JSON_FRAME = {
        "@context": {
            ...
            "validFrom": { "@id": "schema:validFrom" },
            "validTo": { "@id": "schema:validTo" },            
    ...
    const STYLE_SYSTEM = { NODE: { background: '#66BB6A', border: '#000000' }, FONT: { ...FONT_DEFAULTS, color: '#FFFFFF' } };
    ...
    function initGraph() {
        const options = {
            layout: { hierarchical: { enabled: true, direction: 'RL', sortMethod: 'directed', levelSeparation: 300 } },
            nodes: { shape: 'box', margin: 10, widthConstraint: { maximum: 700 }, borderWidth: 1 },
            edges: { width: 1.5, smooth: { type: 'cubicBezier', forceDirection: 'vertical', roundness: 0.4 }, arrows: { to: { enabled: true, scaleFactor: 0.5 } } },
    ...
    function isNodeSystemActive(node) {
        if (!activeFilters.system) return false; 
        ...
        return node.memberships.some(m => {
            ...
            // Re-mapped temporal boundaries to evaluate strict gYear bounds properly
            if (validFromStr) {
                const fromYear = parseInt(validFromStr, 10);
                const fromDate = new Date(fromYear, 0, 1); // January 1st
                if (checkDate < fromDate) return false;
            }
            if (validToStr) {
                const toYear = parseInt(validToStr, 10);
                const toDate = new Date(toYear, 11, 31, 23, 59, 59); // December 31st end of day
    ...
    function refreshGraphColors() {
        ...
        if (selectedIds.length > 0) highlightSelection(selectedIds[0]);
        else resetHighlight();
    }
    ...
    function highlightSelection(selectedNodeId) {
        ...
        const nodesToUpdate = nodes.map(node => {
            ...
            if (highlightedNodeIds.has(node.id)) {
                if (isSystem) return { id: node.id, color: STYLE_SYSTEM.NODE, font: STYLE_SYSTEM.FONT };
    ...
    function resetHighlight() {
        const nodesToUpdate = nodes.map(node => {
            if (isNodeSystemActive(node)) return { id: node.id, color: STYLE_SYSTEM.NODE, font: STYLE_SYSTEM.FONT };
            else return { id: node.id, color: STYLE_NORMAL.NODE, font: STYLE_NORMAL.FONT };
        });
    ...
    function showInfoPanel(nodeId) {
        ...
        infoIri.textContent = `:${compactedId.split('/').pop()}`;
        ...
        if (node.memberships && node.memberships.length > 0) {
            ...
            node.memberships.forEach(membership => {
                ...
                const validFromYear = validFromObj && validFromObj['@value'] ? validFromObj['@value'] : (validFromObj ? validFromObj : 'Start');
                const validToYear = validToObj && validToObj['@value'] ? validToObj['@value'] : (validToObj ? validToObj : 'heute');
        ...
        if (attributes && attributes.length > 0) {
            ...
            for (const [typeName, valueNames] of Object.entries(groupedAttributes)) {
                panelContent += `<p class="attribute-item"><b>${typeName}:</b> ${Array.from(valueNames).join(', ')}</p>`;
            }
